...
4. **Personalization is a 3-question survey.** Optional, fully client-side. Answers live in `localStorage` only. The survey filters the deck to taxes that likely apply.
5. **Estimators are inline.** Each card has a collapsible calculator. Computing one updates the **aggregate banner** at the top of the page.
